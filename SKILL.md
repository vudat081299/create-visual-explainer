...
- Always: `foundations/tokens` + `components/core` + `foundations/pedagogy` + `foundations/structure` + `foundations/verification`, and pick a theme set (`foundations/palettes` + `foundations/backgrounds`) so pages don't all look identical — default: infer from the subject
- **Structure-first is the DEFAULT first build move (rule 17 + `foundations/structure.md`), not a late check.** Before picking components or templates, name the knowledge's intrinsic shape (sequence / comparison / hierarchy / cause→effect / one deep idea / parallel set / decision) and build the macro-layout that makes that relationship visible. Everything below — form choice (rule 13), disclosure (§2D), reader tooling (§2E) — operates *inside* the structure you pick here.
...
### 2B. Content immutable rules (7 rules)
...
17. **Page structure mirrors the shape of the knowledge.** Before reaching for a layout, name the material's intrinsic shape — sequence, comparison, hierarchy, cause→effect, one deep idea, parallel set, decision — then build the macro-layout that makes that relationship *visible*. A process shown as parallel cards, or a comparison shown as sequential prose, is a *structural lie*: the layout-level analogue of a chart that misleads about its data — a truncated axis, a dual axis (`foundations/plot-quality.md` §3–§4). It is to page layout what rule 14 is to plots, and the page-level counterpart to rule 13's per-explanation form choice — think it out, don't template it. (See `foundations/structure.md`.)
...
- ❌ **Macro-layout that contradicts the knowledge's shape** (a process as parallel cards, a comparison as sequential sections, a hierarchy as a flat list) → a *structural lie*, the layout-level counterpart of a truncated axis; name the shape first, then build the layout that expresses it (rule 17, `foundations/structure.md`)
